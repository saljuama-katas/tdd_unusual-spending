...
case class UnusualExpense(category: Category, unusual: BigDecimal, usual: BigDecimal)
...
trait UnusualExpensesAnalyzer {
  ...
  protected def findUnusualExpenses(current: Map[Category, BigDecimal], previous: Map[Category, BigDecimal]): Seq[UnusualExpense] = {
    ...
    current
      ...
      .map { case (category, amount) ⇒ UnusualExpense(category, amount, previous.getOrElse(category, 0.00)) }
      .toSeq
  }
}
